...
async def evaluate_smiles(req: SmilesRequest):
    return rdkit_engine.evaluate_smiles(req.smiles)

@router.post("/dock")
async def run_docking(req: DockingRequest):
    return vina_adapter.run_docking(req.smiles, req.target_pdb_id)
...
async def evaluate_safety_gate(
    ...
    return safety_gate_engine.evaluate_gates(
        herg_liability=herg,
